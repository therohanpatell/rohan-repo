...
class PipelineOrchestrator:
    ...
    def parse_arguments(self) -> argparse.Namespace:
        ...
        parser = argparse.ArgumentParser(  # Set up CLI argument parser
            description='PySpark BigQuery Metrics and DQ Pipeline'
        )
        ...
        parser.add_argument(
            ...
            default='metrics',
            choices=['metrics', 'dq'],
            help='Pipeline mode: "metrics" for metrics pipeline, "dq" for data quality pipeline'
        )
        parser.add_argument(
            '--gcs_path', 
            required=True, 
        ...
        parser.add_argument(
            '--dependencies', 
            required=False, 
            help='Comma-separated list of dependencies to process (required for metrics mode)'
        )
        parser.add_argument(
            '--partition_info_table', 
            required=False, 
            help='BigQuery table for partition info (project.dataset.table) (required for metrics mode)'
        )
        parser.add_argument(
            '--env', 
            required=False, 
            help='Environment name (e.g., BLD, PRD, DEV) (required for metrics mode)'
        )
        parser.add_argument(
            '--recon_table', 
            required=False, 
            help='BigQuery table for reconciliation data (project.dataset.table) (required for metrics mode)'
        )
        parser.add_argument(
            '--dq_target_table',
            required=False,
        ...
        if args.mode == 'metrics':
            if not args.dependencies:
                parser.error("--dependencies is required when mode is 'metrics'")
            if not args.partition_info_table:
                parser.error("--partition_info_table is required when mode is 'metrics'")
            if not args.env:
                parser.error("--env is required when mode is 'metrics'")
            if not args.recon_table:
                parser.error("--recon_table is required when mode is 'metrics'")
        elif args.mode == 'dq':
            if not args.dq_target_table:
                parser.error("--dq_target_table is required when mode is 'dq'")
    ...
    def log_pipeline_info(self, args: argparse.Namespace, dependencies: list = None) -> None:
        ...
        if args.mode == 'metrics':
            ...
            logger.info(f"DQ Target Table: {args.dq_target_table}")
            logger.info("-"*60)
            logger.info("PIPELINE BEHAVIOR:")
    ...
    def execute_dq_pipeline_steps(self, args: argparse.Namespace) -> None:
        """Execute the DQ pipeline steps with comprehensive error handling"""
        logger.info("="*80)
        logger.info("STARTING DQ PIPELINE EXECUTION")
        logger.info("="*80)
        
        with managed_spark_session("DQPipeline") as spark:
            ...
            try:
                # Step 1: Read and validate DQ configuration
                logger.info("-"*60)
                logger.info("STEP 1: READING AND VALIDATING DQ CONFIGURATION")
                logger.info("-"*60)
                logger.info(f"Reading DQ configuration from GCS path: {args.gcs_path}")
                dq_config = dq_pipeline.read_and_validate_dq_config(args.gcs_path)
                logger.info(f"Successfully read and validated {len(dq_config)} DQ checks")
                
                # Count active vs inactive checks
                active_checks = [check for check in dq_config if check.get('active', True)]
                inactive_checks = len(dq_config) - len(active_checks)
                logger.info(f"   Active checks: {len(active_checks)}")
                logger.info(f"   Inactive checks (will be skipped): {inactive_checks}")
                
                # Step 2: Execute DQ checks
                logger.info("-"*60)
                ...
                passed_checks = sum(1 for result in dq_results if result.get('validation_status') == 'PASS')
                failed_checks = len(dq_results) - passed_checks
                logger.info(f"   Passed checks: {passed_checks}")
                logger.info(f"   Failed checks: {failed_checks}")
                
                # Step 3: Write DQ results to BigQuery
                logger.info("-"*60)
                logger.info("STEP 3: WRITING DQ RESULTS TO BIGQUERY")
                logger.info("-"*60)
                logger.info(f"Writing {len(dq_results)} DQ results to table: {args.dq_target_table}")
                dq_pipeline.write_dq_results(dq_results, args.dq_target_table)
                logger.info("DQ results successfully written to BigQuery")
                
                # Log final statistics
                logger.info("-"*60)
                logger.info("FINAL DQ PIPELINE STATISTICS")
                logger.info("-"*60)
                self.log_dq_pipeline_statistics(dq_config, dq_results)
                
                logger.info("="*80)
                logger.info("DQ PIPELINE EXECUTION COMPLETED SUCCESSFULLY")
                logger.info("="*80)
    ...
    def log_dq_pipeline_statistics(self, dq_config: list, dq_results: list) -> None:
        """Log comprehensive DQ pipeline statistics"""
        logger.info("DQ PIPELINE EXECUTION SUMMARY:")
        logger.info(f"   Total DQ checks in configuration: {len(dq_config)}")
        
        # Count active vs inactive
        active_checks = [check for check in dq_config if check.get('active', True)]
        inactive_checks = len(dq_config) - len(active_checks)
        logger.info(f"   Active checks executed: {len(active_checks)}")
        logger.info(f"   Inactive checks skipped: {inactive_checks}")
        ...
        if len(dq_results) > 0:
            success_rate = (passed_checks / len(dq_results)) * 100
            logger.info(f"   Success rate: {success_rate:.1f}%")
        
        # Group by severity
        if failed_checks > 0:
            logger.warning("FAILED CHECKS BY SEVERITY:")
            severity_counts = {}
            ...
            for severity, count in sorted(severity_counts.items()):
                logger.warning(f"   {severity}: {count} failed checks")
        
        # Group by category
        logger.info("CHECKS BY CATEGORY:")
        category_stats = {}
        for result in dq_results:
        ...
        for category, stats in sorted(category_stats.items()):
            ...
            logger.info(f"   {category}: {total} checks ({stats.get('PASS', 0)} passed, {stats.get('FAIL', 0)} failed)")
...
def main():
    ...
    try:
        ...
        if args.mode == 'metrics':
            ...
            print("="*80)
            print("DQ PIPELINE STARTING")
            print("="*80)
            
            # Log pipeline information
            logger.info("Logging pipeline configuration...")
            orchestrator.log_pipeline_info(args)  # Log configuration details
            
            # Execute DQ pipeline
            logger.info("Starting DQ pipeline execution...")
            orchestrator.execute_dq_pipeline_steps(args)  # Run DQ pipeline logic
        ...
        pipeline_name = "METRICS PIPELINE" if args.mode == 'metrics' else "DQ PIPELINE"
        print("="*80)
        print(f"{pipeline_name} FAILED - KNOWN ERROR")
        print("="*80)
        ...
        pipeline_name = "METRICS PIPELINE" if args.mode == 'metrics' else "DQ PIPELINE"
        print("="*80)
        print(f"{pipeline_name} FAILED - UNEXPECTED ERROR")
        print("="*80)
