...
class BigQueryOperations:
    ...
    def execute_query(self, query: str, timeout: int = PipelineConfig.QUERY_TIMEOUT) -> bigquery.table.RowIterator:
        ...
        try:
            ...
            logger.info(f"Query: {query}")  # Changed from debug to info for easier debugging
    ...
    def execute_sql_with_results(self, sql: str, metric_id: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            if result_dict['denominator_value'] is not None:
                try:
                    ...
                    if denominator_decimal is not None:
                        ...
                        if denominator_decimal < 0:
                            error_msg = f"Invalid denominator value: denominator value is negative ({denominator_decimal}). Negative denominators are not allowed."
                            if metric_id:
                                error_msg = f"Metric '{metric_id}': {error_msg}"
                            logger.error(error_msg)
                            raise MetricsPipelineError(error_msg)
            ...
            logger.info(f"Query results (processed): {result_dict}")  # Log the processed result dictionary
            return result_dict
    ...
    def delete_metrics(self, metric_ids: List[str], partition_dt: str, target_table: str) -> None:
        ...
        try:
            ...
            # Try to get number of affected rows (not always available)
            try:
                ...
                logger.info("Successfully executed delete query for overwrite operation")
